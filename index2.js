...
function startCamera(condition) {
    if (condition) {
...
// Function to reset face detection 
...
recognition.onend = () => {
    ...
    setTimeout(() => recognition.start(), 500); // Small delay to avoid infinite loops
};
...
recognition.onresult = (event) => {
    ...
    if(transcript.toLowerCase().includes('hello maria')|transcript.toLowerCase().includes('hello, maria')){
        startCamera(true);
        speak("Initializing Mariya")
    }
        if (transcript.toLowerCase().includes("stop")||transcript.toLowerCase().includes("stop.")) {
...
navigator.mediaDevices.getUserMedia({
    ...
    input.connect(analyser); // Connect mic input to recognition
}).catch((err) => {
...
//GRAPHIC ERA UNICERSITY ALL QUESTIONS
    ...
    else if (command.includes("where is Graphic Era University located")|| command.includes("graphic era located")) {
